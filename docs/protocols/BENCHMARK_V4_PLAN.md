...
3. **Notebook B — local models:** GPU session for Von/Laya and frozen embeddings; CPU-limited SVM/CatBoost/AutoML jobs. Run heavy local jobs sequentially by default. If AutoML requires incompatible packages, use an isolated environment within this notebook before starting work. The two T4s are not pooled memory; assign separate devices only after each workload is verified to fit.
